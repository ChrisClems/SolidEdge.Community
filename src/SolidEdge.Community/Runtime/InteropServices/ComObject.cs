...
public class ComObject
{
    ...
    {
        if (System.Runtime.InteropServices.Marshal.IsComObject(comObject) == false)
            throw new InvalidComObjectException();

        var dispatch = comObject as IDispatch;

        if (dispatch != null) return dispatch.GetTypeInfo(0, LOCALE_SYSTEM_DEFAULT);
    ...
    public static T GetPropertyValue<T>(object comObject, string name)
    {
        ...
        var value = type.InvokeMember(name, BindingFlags.GetProperty, null, comObject, null);
    ...
    public static Type GetType(object comObject)
    {
        ...
        try
        {
            if (dispatch != null)
            {
                ...
                typeAttr = (TYPEATTR)System.Runtime.InteropServices.Marshal.PtrToStructure(pTypeAttr, typeof(TYPEATTR));
